...
const { set, pick, prop, isArray, differenceWith, differenceBy } = require('lodash/fp');
...
const create = async attributes => {
  ...
  const autoGeneratedCode = `${kebabCase(attributes.name)}-${generateTimestampCode()}`;
...
const update = async (params, attributes) => {
  const sanitizedAttributes = omit(attributes, ['code']);

  if (has(params, 'id') && has(sanitizedAttributes, 'name')) {
